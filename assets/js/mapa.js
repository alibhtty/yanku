...
function initializeMap() {
    var mapElement = document.getElementById('map_63509cfb9aa56');
    var centerLat = parseFloat(mapElement.getAttribute('data-center-lat'));
    var centerLng = parseFloat(mapElement.getAttribute('data-center-lng'));
    var zoomLevel = parseInt(mapElement.getAttribute('data-zoom-level'));

    // Estilo para ocultar puntos de interés comerciales y otros
    var mapStyle = [
        /* { "elementType": "geometry", "stylers": [{ "color": "#FFFFE0" }] }, */
        { "featureType": "road.local", "stylers": [{ "color": "#cccccc" }] }, //calles
        { "featureType": "water", "elementType": "geometry", "stylers": [{ "color": "#a0d8eb" }] },
    ...
    markerContainer.className = 'heartbeat-container';

    var markerOuter = document.createElement('div');
    markerOuter.className = 'heartbeat';

    var markerInner = document.createElement('div');
    markerInner.className = 'heartbeat-inner';

    markerContainer.appendChild(markerOuter);
    markerContainer.appendChild(markerInner);

    var customMarker = new google.maps.OverlayView();
    customMarker.onAdd = function() {
        var panes = this.getPanes();
        panes.overlayLayer.appendChild(markerContainer);
    };
    customMarker.draw = function() {
        var overlayProjection = this.getProjection();
        var position = overlayProjection.fromLatLngToDivPixel(new google.maps.LatLng(centerLat, centerLng));
        markerContainer.style.left = (position.x - 22) + 'px'; // Ajusta la posición para centrar el contenedor
        markerContainer.style.top = (position.y - 22) + 'px'; // Ajusta la posición para centrar el contenedor
    };
